...
const portraitCards = [
  ...
  { src: '/vaishnava_portrait.png', rotate: 0, x: 0, y: 0, scale: 1, z: 4 },
    ...
    <section className="relative min-h-[90vh] flex flex-col items-center overflow-hidden bg-[#faf9f6] pt-32 pb-20">
      ...
      <div className="z-10 container mx-auto px-4 text-center max-w-5xl mb-16 relative">
        ...
        >
          ...
          <br />
          <span className="text-orange-600 italic font-medium">{copy.hero.titleLine2}</span>
        </motion.h1>
        ...
        >
          {copy.hero.descriptionPrefix}
          <span className="font-semibold text-[#2c1810]">{copy.hero.descriptionHighlight1}</span>
          {copy.hero.descriptionMiddle}
          <span className="font-semibold text-[#2c1810]">{copy.hero.descriptionHighlight2}</span>
          {copy.hero.descriptionSuffix}
        </motion.p>
        ...
        >
          ...
          <Link href="#features" className="flex items-center gap-3 bg-white text-[#2c1810] px-8 py-4 rounded-2xl hover:bg-[#faf9f6] transition-all hover:border-orange-200 border border-[#e7e5e4] shadow-md group">
            ...
            <span className="font-bold text-lg">{copy.hero.secondaryCta}</span>
          </Link>
        </motion.div>
      </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          {portraitCards.map((card, idx) => (
            <motion.div
              ...
              animate={{ opacity: 1, y: card.y, x: card.x, scale: card.scale, rotate: card.rotate }}
              whileHover={{ scale: card.scale * 1.1, y: card.y - 20, zIndex: 10, transition: { duration: 0.3 } }}
            ...
            >
              <Image src={card.src} alt={`Portrait ${idx + 1}`} fill className="object-cover hover:scale-110 transition-transform duration-700" />
            </motion.div>
          ))}
        </div>
