...
const galleryImages = [
  { src: '/vaishnava_group.png', style: { top: '5%', left: '5%' }, rotate: -6 },
  { src: '/krishna_bg.png', style: { top: '10%', right: '5%' }, rotate: 5 },
  { src: '/vaishnava_portrait.png', style: { top: '40%', left: '2%' }, rotate: 3 },
  { src: '/krishnaAssistant.png', style: { top: '45%', right: '2%' }, rotate: -4 },
  { src: '/vaishnava_group.png', style: { bottom: '5%', left: '10%' }, rotate: -5 },
  { src: '/vaishnava_portrait.png', style: { bottom: '10%', right: '8%' }, rotate: 6 },
];

export function ScrollSection({ copy }: { copy: LandingCopy }) {
    ...
    <section ref={containerRef} className="py-32 bg-[#fffbeb] overflow-hidden min-h-screen flex items-center justify-center relative">
      ...
      <div className="container mx-auto px-4 relative z-30 text-center pointer-events-none">
        <div className="bg-[#fffbeb]/80 backdrop-blur-md p-8 md:p-12 rounded-[2rem] inline-block shadow-sm pointer-events-auto border border-white/20">
          ...
          <p className="text-[#5c4d47] max-w-xl mx-auto text-lg leading-relaxed">{copy.scroll.description}</p>
        </div>
      </div>

      <div className="absolute inset-0 pointer-events-none z-0">
        {galleryImages.map((img, idx) => {
          const yMove = idx % 2 === 0 ? y : yReverse;

          return (
            <motion.div
              key={idx}
              style={{ y: yMove, rotate: img.rotate, ...img.style }}
            ...
            >
              <Image src={img.src} alt={copy.scroll.imageAlts[idx]} fill className="object-cover hover:scale-110 transition-transform duration-700" />
            </motion.div>
          );
        })}
        ...
        <div className="md:hidden grid grid-cols-2 gap-4 px-4 w-full">
          {galleryImages.slice(0, 4).map((img, idx) => (
            <div key={idx} className="aspect-[3/4] relative rounded-xl overflow-hidden shadow-lg border-2 border-white">
              <Image src={img.src} alt={copy.scroll.imageAlts[idx]} fill className="object-cover" />
            </div>
          ))}
        </div>
      </div>
