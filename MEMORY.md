...
- Source-of-truth для production Union web — `feat/union-web-prod`; `main` и `feat/vedabase` не должны использоваться как deploy source для Union. Docker build вызывает `scripts/verify-union-web-source.sh`, который падает, если в source нет актуального Union landing, `/app -> /app/union`, `/app/union/*`, requests badge/list flow, `UnionNotice`/`UnionSkeleton` и location suggestions.
- Перед любыми правками Union web нужно быть на ветке `feat/union-web-prod` и проверять это командой `git status --short --branch`. `main` можно синхронизировать после готовой проверки, но не использовать как рабочую ветку для Union; `feat/vedabase` не деплоить напрямую в Union, а переносить из нее только осознанно выбранные web/shared frontend изменения.
- 2026-06-16 актуальный Union web/cabinet был найден не в старом `feat/union-web-prod`, а в `feat/vedabase`; в production-ветку перенесен только web/shared frontend surface (`apps/web`, `packages/api-client`, `packages/domain-types`, `packages/i18n`, `pnpm-lock.yaml`) без server Vedabase backend. Новый production Union больше не использует старые `/app/dating/*` routes; canonical cabinet routes: `/app/union`, `/app/union/profile`, `/app/union/requests`.
...
- Для web Union в `apps/web` актуальный кабинет живет на `/app/union`, `/app/union/profile`, `/app/union/requests`; старый web-only surface `/app/dating/*` больше не является production path. Backend contracts для Union остаются в `/dating/*` и media endpoints, но UI routes должны идти через `/app/union/*`.
