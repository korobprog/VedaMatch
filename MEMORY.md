...
- Для web Union в `apps/web` canonical cabinet route снова `/app/union`, как на live `union.vedamatch.ru`; новые фичи профиля/фото/лайков/встреч продолжают использовать текущие backend contracts `/dating/profile/:id`, `/dating/profile/:id/submit`, `/media/upload/:userId`, `/media/:id/set-profile` и не требуют отдельного web-only photo backend.
- Локальный запуск Union web идет через `pnpm --dir apps/web run dev`; runtime слушает `http://localhost:3007`.
- Для local `apps/web` на `http://localhost:3007` API base должен быть `http://localhost:8000/api`; backend CORS defaults должны включать `http://localhost:3007` и `http://127.0.0.1:3007`, иначе login/profile/content routes упираются в `ERR_CONNECTION_REFUSED` или CORS preflight.
- Если `http://localhost:3007/login` показывает browser error `ERR_CONNECTION_REFUSED`, сначала проверить `lsof -nP -iTCP:3007 -sTCP:LISTEN`; рабочий локальный запуск web: detached `screen -dmS vedicai-web-3007 zsh -lc 'cd /Users/mamu/Documents/vedicai/apps/web && pnpm run dev > /Users/mamu/Documents/vedicai/logs/local-web-3007.log 2>&1'`.
- Если local `apps/web` на `3007` периодически отдает Next dev 500 вроде `module factory is not available` или `useSession must be used within SessionProvider` при корректном `RootLayout`, это может быть битое Turbopack/Fast Refresh состояние. Проверить `logs/local-web-3007.log`, закрыть screen, убедиться через `lsof -nP -iTCP:3007 -sTCP:LISTEN`, что не остался orphan `next-server`, при необходимости убить только этот PID и заново поднять `vedicai-web-3007`.
- Для mobile Union nav нельзя скрывать все `span` внутри `.union-nav__link`: иконка обернута в `span.union-nav__icon`, поэтому mobile CSS должен скрывать только `.union-nav__link > span:not(.union-nav__icon)`, а не саму иконку. Зеленые Union `dashboard-action` могут иметь одновременно `dashboard-action--ghost`, поэтому цвет текста/иконок на зеленом фоне нужно задавать для `.shell--union .dashboard-action` целиком.
- Для разделения Union и общего VedaMatch web нельзя определять Union как весь `social` surface: `union.vedamatch.ru` / `.com` и local `localhost` должны считаться `isUnion`, а общий `social.vedamatch.*` должен оставаться social launcher. На Union `/` рендерит `UnionPublicHome`, `/app` редиректит в `/app/union`, а `/app/*` оборачивается в отдельный `UnionAppFrame` без общего VedaMatch launcher-а.
- Источник путаницы найден в git: коммит `99109436 feat: deploy web union surface` на ветке `fix/inspiration-and-dating-mode` имел отдельный Union cabinet (`/app/union`, `UnionAppFrame`), но текущий `main` его потерял и оставил `/app` на общем VedaMatch/social cabinet или новом `/app/dating` пути.
- 2026-06-14 восстановление live-style Union cabinet локально:
  - `/app` для Union/local снова редиректит на `/app/union`;
  - `/app/union`, `/app/union/profile`, `/app/union/requests` существуют локально и используют отдельный `UnionAppFrame`;
  - `UnionDashboard` адаптирован к текущим `datingWeb`, `getDatingCandidates` и `getWallet(baseUrl, accessToken)`;
  - `UnionProfileForm` является отдельной web-анкетой для `/app/union/profile`, а `UnionRequests` служит совместимым alias на `UnionLikes`;
  - новые `/app/dating/*` routes сохраняются для кандидата, browse, likes, meetings, но открываются под Union shell.
- Для локальной проверки именно production build Union web: `pnpm --filter @vedamatch/web build`, затем standalone runtime `PORT=3007 HOSTNAME=0.0.0.0 node apps/web/.next/standalone/apps/web/server.js`; перед standalone запуском нужно иметь скопированные `apps/web/.next/static` и `apps/web/public` внутрь `apps/web/.next/standalone/apps/web/`.
- Если на `3007` поднялся старый `next-server`, его лучше перезапустить перед проверкой UI, иначе можно увидеть старый рендер или stale state.
- Для публичного Union landing в `apps/web/src/components/union-public-home.tsx` язык должен читаться из `SessionProvider`, а не только из server prop; иначе `LanguageSwitcher` меняет cookie/localStorage/select, но hero-copy не обновляется до reload/новой сборки.
- Если пользователь говорит, что локально видит кошелек вместо Union-знакомств, проверить `3006`: там обычно висит LKM. Для устранения путаницы остановить LKM на `3006` и запустить Union web standalone на `3006`; целевой Union cabinet URL теперь `/app/union`, без auth он редиректит на `/login`.
- Если локально на Union root видна старая generic-страница `Shared contracts first`, это не web-контейнер: root `apps/web/src/app/page.tsx` должен рендерить публичный Union landing для `social/union` surface и для local standalone. `union.vedamatch.ru` / `.com` должны резолвиться в `social` surface, а local standalone на `3006/3007` может брать публичную статистику `/dating/presentation` через fallback `https://api.vedamatch.ru/api`, если локальный API на `8000` недоступен.
- Для production-ready Union web в `apps/web` root metadata должны быть host-aware: `union.vedamatch.ru` / `.com` получают title `Союз | VedaMatch`, Union description, canonical/OpenGraph/Twitter URL на `https://<host>`, а non-public routes закрываются через `robots.txt` (`/app/`, `/login`, `/register`) и single-entry `sitemap.xml`.
- `getRequestSurface()` должен хранить две формы host context: normalized `host` для surface/language detection и полный `origin` для metadata/robots/sitemap. Для localhost/127.0.0.1 origin оставляет `http` и порт, для публичных host canonical должен быть `https`, даже если локальный smoke идет через внутренний HTTP.
- Next.js production hardening для `apps/web`: `poweredByHeader: false`, security headers через `next.config.ts` (`Strict-Transport-Security`, `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, restrained `Permissions-Policy`), `optimizePackageImports` для `lucide-react`, без wildcard `images.remotePatterns`.
- 2026-06-14 Union web production route: `union.vedamatch.ru` в Traefik обслуживается не `vedamatch-vedamatchsocial-zcxupc`, а отдельным Dokploy/Swarm service `app-override-open-source-alarm-gmfjt2` через `/etc/dokploy/traefik/dynamic/app-override-open-source-alarm-gmfjt2.yml`.
- 2026-06-14 actual Union web service `app-override-open-source-alarm-gmfjt2` обновлен вручную на VPS из `/etc/dokploy/applications/app-override-open-source-alarm-gmfjt2/code`: добавлены host-aware metadata, `robots.txt`, `sitemap.xml`, security headers, `poweredByHeader: false`, restricted `next/image` patterns; Docker image `app-override-open-source-alarm-gmfjt2:latest` собран и `docker service update --force` сошелся.
- Live smoke после deploy `https://union.vedamatch.ru`: root отдает title `Союз | VedaMatch`, `x-powered-by` отсутствует, security headers есть, `/robots.txt` и `/sitemap.xml` отвечают `200`. Риск остается инфраструктурный: GitHub push по-прежнему заблокирован HTTPS auth/отсутствием `gh`, а server-side hotfix живет в Dokploy working tree и может быть перетерт будущим redeploy из GitHub, пока remote source не синхронизирован.
...
- Mobile Union profile source-of-truth: `frontend/screens/portal/dating/EditDatingProfileScreen.tsx` сохраняет в `PUT /dating/profile/:id` поля `city`, `bio`, `interests`, `lookingFor`, `maritalStatus`, `dob`, `birthTime`, `birthPlaceLink`, `madh`, `yogaStyle`, `guna`, `identity`, `datingEnabled`, `intentions`, `skills`, `industry`, `lookingForBusiness`, `childrenIntent`, `loveLanguages`, `elementalPrimary`, `elementalSecondary`, `meetingPreferences`, `socialLinks`; profile posts создаются отдельно через `/dating/posts`.
- Mobile services source-of-truth: `frontend/screens/portal/services/CreateServiceScreen.tsx` создает/редактирует услугу через `/services` с `title`, `description`, `coverImageUrl`, `category`, `channel`, `accessType`, `channelLink`, `offlineAddress`, `formats: ["individual"]`, `scheduleType: "booking"` и тарифами `name`, `price`, `durationMinutes`, `sessionsCount`, `isDefault`; расписание редактируется отдельно в `ServiceScheduleScreen` через `/services/:id/schedule/weekly`.
- Mobile registration has two overlapping flows: `RegistrationScreen` initial phase creates `/register` with `email`, `password`, `invite_code`, `role`, `godModeEnabled`, `deviceId`, then navigates to `RoleDetail -> RoleProfileForm`, where registration context posts `/register` again with full profile fields. Backend `Register` ignores public `godModeEnabled` and expects `inviteCode` camelCase, so initial `invite_code` may not link referrals.
...
- Для web Union photo gallery на `union.vedamatch.ru` актуальный подход такой:
  - scope только `apps/web`/shared web packages, не React Native и без записи в `Docs/IOS_CHANGES_FOR_MIGRATION.md`;
  - backend media endpoints уже существуют и переиспользуются без schema changes: `GET /media/:userId`, `POST /media/upload/:userId`, `POST /media/:id/set-profile`, `DELETE /media/:id`;
  - новая загрузка отредактированного фото продолжает автоматически делать его главным, затем refresh профиля/галереи;
  - web UI должен поддерживать галерею, просмотр фото, set-main/delete, empty/loading/error states и локализацию RU/EN/HI.
- В `apps/web/src/components/union-profile-form.tsx` city selector должен оставаться controlled combobox: список городов из `GET /dating/cities` не раскрывается при загрузке профиля или простом фокусе, а открывается только после ввода или явного клика по кнопке раскрытия.
- В web-анкете `/app/union/profile` пользователь не должен вводить справочные поля совсем произвольно: `city` и `country` остаются combobox, `madh` и `identity` — select со справочниками как в mobile; legacy/current значения нужно подмешивать в options, чтобы не потерять уже сохраненные данные.
- Production-подход для location suggestions в `apps/web`: client не должен импортировать большой справочник стран/городов; подсказки идут через same-origin route handlers `/api/locations/countries` и `/api/locations/cities`. `countries` строится server-side через `Intl.DisplayNames` + fallback/aliases, `cities` server-side проксирует и кэширует backend `GET /dating/cities` с bearer token текущей web-сессии. Полный мировой справочник городов можно добавить позже на backend/server-side, не в browser bundle.
- Для `/app/union/profile` option labels города/страны/традиции/идентичности локализуются через `packages/i18n` на `ru/en/hi`, но сохраняемые values остаются backend-stable (`Moscow`, `Russia`, `ISKCON`, `Yogi` и т.д.), чтобы язык UI не менял данные в БД. City search aliases держать server-side в `apps/web/src/lib/location-suggestions.ts`, чтобы ввод вроде `моск` мог найти backend value `Moscow`.
- City suggestions в `/app/union/profile` не должны зависеть только от access token и опубликованных backend-анкет: `apps/web/src/lib/location-suggestions.ts` держит server-side fallback city list из mobile-паттерна и объединяет его с `GET /dating/cities`, поэтому `/api/locations/cities?q=моск` должен отдавать `Moscow` даже без авторизации.
- Фильтры `/app/union` должны использовать тот же controlled UX, что и анкета: `city` как combobox с `/api/locations/cities`, `madh` и `identity` как select с локализованными labels из `union.optionLabels`; в backend query уходят стабильные values.
- 2026-06-14 `/app/union/profile` содержит web-only фото flow без редактора изображений: `@vedamatch/api-client` оборачивает `GET /dating/cities`, `GET/POST/DELETE /media`, `POST /media/:id/set-profile`; upload нового фото автоматически делает его главным и refresh-ит галерею.
- После web Union photo gallery implementation 2026-06-13 проверки `pnpm --filter @vedamatch/web typecheck` и `pnpm --filter @vedamatch/web build` прошли; локальный `/app/dating` отдаёт `200 OK`, но реальный upload/delete требует авторизованной браузерной проверки с live/staging API.
...
- 2026-06-14 local Union web smoke:
  - `apps/web` работает на `http://localhost:3007`, backend нужен на `http://localhost:8000`;
  - если `/login` показывает `Failed to fetch`, сначала проверить `curl http://localhost:8000/api/login` / CORS preflight с origin `http://localhost:3007`; локальный web/admin fallback должен смотреть на `http://localhost:8000/api`, не на старый `8081`;
  - если `test37@test.ru` дает `Invalid password`, это локальная DB password mismatch; аккаунт существует как active `devotee`, профиль не завершен, пароль можно безопасно сбросить для dev-входа через bcrypt update в local Postgres;
  - Docker `rag-agent-server` может падать отдельно из-за compose/DNS `postgres`; для локального web smoke надежнее запускать Go backend напрямую через `PORT=8000 go run cmd/api/main.go` из `server/`;
  - invalid login корректно показывает auth error; register через browser UI создает локального пользователя и редиректит в `/app/profile`;
  - `PUT /api/update-profile` падал из-за устаревших column keys `nickname_change_cooldown_until` / `role_change_cooldown_until`; правильные DB columns по текущей модели: `nickname_cooldown_until` / `role_cooldown_until`;
  - после фикса базовый `/app/profile` сохраняется через UI; backend нормализует city `Москва` -> `Moscow`, nickname hyphen -> underscore-safe value;
  - Union draft card через `/app/dating` сохраняется из browser UI, reload страницы подтверждает persistence `dob`, `birthTime`, `bio`, `childrenIntent`, `elementalPrimary`, `loveLanguages`;
  - actual photo upload smoke через browser UI и `react-filerobot-image-editor`: file chooser принимает PNG, editor рендерит `320 × 320`, dialog `Save as` сохраняет webp, upload делает фото главным, gallery count становится `1`, submit-review разблокируется;
  - submit-review через UI переводит локальную анкету в `pending_admin_review`, показывает RU status `На проверке модератора` и локализованную fallback reason `Недостаточно друзей — анкета будет проверена модератором.`;
  - routes `/app/dating/browse`, `/likes`, `/meetings` открываются под auth-сессией, имеют Union nav и H1 `Поиск знакомств` / `Симпатии` / `Встречи`, старый текст `Core web shell` не появляется;
  - `apps/web/src/app/favicon.ico/route.ts` добавлен как SVG-backed handler; local dev `/favicon.ico` теперь `200`, `typecheck` и `build` включают handler в standalone output;
  - console finding: после открытия Filerobot editor в dev console появляются styled-components/React warnings about unknown DOM props (`showTabsDrawer`, `noMargin`, `iconShadow`, etc.) из `react-filerobot-image-editor`;
  - после восстановления live-style cabinet локально `http://localhost:3007/app` после гидрации редиректит на `/app/union`; `/app/union`, `/app/union/profile`, `/app/dating/likes`, `/app/dating/meetings` открываются под `shell--union`;
  - live production source, похоже, расходится с local source: live `/login` уже использует `entryVariant="union"` и copy `Войдите, чтобы искать людей по ценностям...`, а local source в текущем worktree использует более общий auth copy; перед production deploy нельзя слепо перетирать Dokploy working tree без merge/parity review;
  - deploy blockers: Dokploy MCP возвращает HTML вместо JSON, VPS read-only MCP падает `Transport closed`, `GIT_TERMINAL_PROMPT=0 git push --dry-run origin main` падает `could not read Username for 'https://github.com'`;
  - auth `/login` и `/register` больше не используют generic `Core web shell` subtitle, а берут Union-specific localized `auth.loginBody` / `auth.registerBody`;
  - known backend fallback reason `Not enough friends for 3 approvals. Sent to admin review.` на web локализуется через `copy.publication.needsAdminFallback`, чтобы RU UI не показывал английский service text.

## LKM Web
- Старый LKM-сайт локально запускается через `pnpm --dir lkm run dev` и слушает `http://localhost:3006`.
- Актуальный Go backend по `server/cmd/api/main.go` слушает `http://localhost:8000`; local fallback LKM API должен смотреть на `http://localhost:8000/api`, если `NEXT_PUBLIC_API_URL` не задан.
- Telegram WebApp script при локальном Next dev добавляет inline style на `<html>` до гидрации; для `lkm/src/app/layout.tsx` нужен `suppressHydrationWarning`, чтобы dev-overlay не перекрывал страницу hydration mismatch.
