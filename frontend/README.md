...
### Feature flags (оптимизация)

Доступные флаги в `.env`:

- `FF_QUERY_LAYER=true` — флаг query-слоя (TanStack Query).
- `FF_FLASHLIST_NEWS=true` — использовать FlashList на экране новостей.
- `FF_FLASHLIST_SERVICES=true` — использовать FlashList на экране сервисов.
