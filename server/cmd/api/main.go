...
func main() {
	...
	services.StartAdminPushCampaignScheduler()
	...
	ekadashiService := services.NewEkadashiService()
	...
	callDiagnosticsHandler := handlers.NewCallDiagnosticsHandler()
	...
	ekadashiHandler := handlers.NewEkadashiHandlerWithService(ekadashiService)
	connectHandler := handlers.NewConnectHandler()
	...
	app.Get("/auth/telegram/callback", middleware.RateLimitByIP("auth_telegram_callback_alias", 120, 10*time.Minute), authHandler.TelegramMobileCallback)
	...
	api.Post("/channels/:channelId/posts/:postId/view", middleware.OptionalAuth(), channelHandler.TrackPostView)
	api.Post("/channels/:channelId/posts/:postId/share", middleware.OptionalAuth(), channelHandler.TrackPostShare)
	api.Get("/channels/:channelId/posts/:postId/comments", middleware.OptionalAuth(), channelHandler.ListPostComments)
	...
	api.Get("/connect/feed", middleware.OptionalAuth(), connectHandler.GetFeed)
	api.Get("/connect/opportunities/:id", middleware.OptionalAuth(), connectHandler.GetOpportunity)
	api.Get("/connect/communities/:id", middleware.OptionalAuth(), connectHandler.GetCommunity)
	...
	publicCharity.Get("/projects/:id", charityHandler.GetProjectByID)
	...
	admin.Post("/push/campaigns", adminHandler.CreatePushCampaign)
	admin.Get("/push/campaigns", adminHandler.ListPushCampaigns)
	admin.Get("/push/campaigns/:id", adminHandler.GetPushCampaign)
	admin.Post("/push/campaigns/:id/cancel", adminHandler.CancelPushCampaign)
	...
	protected.Put("/connect/profile", connectHandler.UpsertProfile)
	protected.Post("/connect/opportunities", connectHandler.CreateOpportunity)
	protected.Post("/connect/opportunities/:id/apply", connectHandler.Apply)
	protected.Get("/connect/opportunities/:id/applications", connectHandler.ListApplications)
	protected.Post("/connect/opportunities/:id/feedback", connectHandler.SubmitFeedback)
	protected.Post("/connect/applications/:id/status", connectHandler.UpdateApplicationStatus)
	admin.Get("/connect/opportunities", connectHandler.ListPendingOpportunities)
	admin.Post("/connect/opportunities/:id/approve", connectHandler.ApproveOpportunity)
	admin.Post("/connect/opportunities/:id/reject", connectHandler.RejectOpportunity)
	...
	protected.Post("/channels/:id/live/:liveId/start", channelHandler.StartLiveSession)
	protected.Post("/channels/:id/live/:liveId/end", channelHandler.EndLiveSession)
	protected.Post("/channels/:id/live/:liveId/cancel", channelHandler.CancelLiveSession)
	...
	protected.Post("/calls/diagnostics", callDiagnosticsHandler.Create)
